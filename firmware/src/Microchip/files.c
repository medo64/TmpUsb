...
//definition in fileio_config.h.
#if defined(__XC8)
    #define MBR_ADDR    DRV_FILEIO_INTERNAL_FLASH_CONFIG_FILES_ADDRESS
    #define BOOT_SEC_ADDR   MBR_ADDR + FILEIO_CONFIG_MEDIA_SECTOR_SIZE
    #define FAT_TBL_ADDR    BOOT_SEC_ADDR + FILEIO_CONFIG_MEDIA_SECTOR_SIZE
    #define ROOT_DIR_ADDR   FAT_TBL_ADDR + FILEIO_CONFIG_MEDIA_SECTOR_SIZE
    #define FILE_DATA_ADDR  ROOT_DIR_ADDR + (FILEIO_CONFIG_MEDIA_SECTOR_SIZE * (DRV_FILEIO_CONFIG_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT / 16))
    #define OTHER_FILE_PLACEHOLDER_ADDR     FILE_DATA_ADDR + FILEIO_CONFIG_MEDIA_SECTOR_SIZE
    ...
    #define AT(address)
    #define MBR_ADDR
    #define BOOT_SEC_ADDR
    #define FAT_TBL_ADDR
    #define ROOT_DIR_ADDR
    #define FILE_DATA_ADDR
    #define OTHER_FILE_PLACEHOLDER_ADDR
#endif
...
//------------------------------------------------------------------------------
const uint8_t MBR_ATTRIBUTES MasterBootRecord[FILEIO_CONFIG_MEDIA_SECTOR_SIZE] AT(MBR_ADDR) =
{
//Code Area
...
const uint8_t PARTITION_ATTRIBUTES(BOOT_SECTOR_ADDRESS) BootSector[FILEIO_CONFIG_MEDIA_SECTOR_SIZE]  AT(BOOT_SEC_ADDR) =
{
...
const uint8_t PARTITION_ATTRIBUTES(FAT0_ADDRESS) FAT0[FILEIO_CONFIG_MEDIA_SECTOR_SIZE]  AT(FAT_TBL_ADDR) =
{
    //Format of each line:
...
const uint8_t PARTITION_ATTRIBUTES(ROOTDIRECTORY0_ADDRESS) RootDirectory0[FILEIO_CONFIG_MEDIA_SECTOR_SIZE]  AT(ROOT_DIR_ADDR) =
{
    //Root
...
const uint8_t PARTITION_ATTRIBUTES(SLACK0_ADDRESS) slack0[FILEIO_CONFIG_MEDIA_SECTOR_SIZE]  AT(FILE_DATA_ADDR) =
{
    'D','a','t','a'
};
#endif
...
#if defined(__XC8)
    const uint8_t PARTITION_ATTRIBUTES(SLACK1_ADDRESS) slack1[FILEIO_CONFIG_MEDIA_SECTOR_SIZE*(DRV_FILEIO_INTERNAL_FLASH_CONFIG_DRIVE_CAPACITY - 1u)] AT(OTHER_FILE_PLACEHOLDER_ADDR) = {0};
#else
